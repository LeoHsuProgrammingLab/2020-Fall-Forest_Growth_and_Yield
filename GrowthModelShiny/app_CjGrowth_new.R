...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
     ...
     column(4,selectInput("select", h3("Type"), 
   ...
   fluidRow(
     column(6, numericInput("Area", h3("Stand Area (hectare)"), value = 0.153))
   ),  
   fluidRow(
     column(3, numericInput("StartAge", h3("Start Age (year)"), value = 10)),
     column(3, numericInput("EndAge", h3("End Age (year)"), value = 100))
     ), 
   fluidRow(
...
server <- function(input, output) {
  ...
  CjGY <- function(DBH, D, startage, endage, Area, sd_DBH_, sd_Mor_, sd_H_){
    ...
    stand_attribute <- data.frame("Age"=startage,"QMD"=sqrt(mean(DBH^2, na.rm = T)),"Stand_height"=H_mean_now,"Den"=D/Area,"Basal_area"=sum(pi*(DBH/200)^2, na.rm = T)/Area,"Stand_vol"=sum(0.00007854*DBH^2*H_now*0.45, na.rm = T)/Area)
    ...
    for (i in startage:(endage-1)) {
      ...
      stand_attribute[nrow(stand_attribute)+1,] <- c(i+1, sqrt(mean(DBH_next^2, na.rm = T)), mean(H_next, na.rm = T), D_next/Area, sum(pi*(DBH_next/200)^2, na.rm = T)/Area, sum(0.00007854*DBH_next^2*H_next*0.45, na.rm = T)/Area)
  ...
  plotting <- eventReactive(input$button, {
  ...
  output$plot <- renderPlot({
    ...
    if (input$select==1){
      plot(values$sim_growth$Age, values$sim_growth$Stand_vol, 
           xlab = "Age (yr)", 
           ylab = expression(paste("Predicted volume ( ", m^3, "/ ha)")),
           cex.lab = 2.5,
           cex.axis = 2,
           cex = 1.2,
           pch = 19)
    ...
    if (input$select==2){
      plot(values$sim_growth$Age, values$sim_growth$Basal_area, 
           xlab = "Age (yr)", 
           ylab = expression(paste("Predicted basal area ( ", m^2, "/ ha)")),
           cex.lab = 2.5,
           cex.axis = 2,
           cex = 1.2,
           pch = 19)
    ...
    if (input$select==3){
      plot(values$sim_growth$Age, values$sim_growth$Den, 
           xlab = "Age (yr)", 
           ylab = "Predicted stand density (stems/ha)",
           cex.lab = 2.5,
           cex.axis = 2,
           cex = 1.2,
           pch = 19)
